...
export async function getNextContractCodeNumber(
  tx: Prisma.TransactionClient,
  model: "pawnContract" | "unsecuredContract" | "installmentContract",
  prefix: string
): Promise<number> {
  const records = await (tx as any)[model].findMany({
    where: {
...
export async function generateContractCode(
  ...
  let prefix = "";
  let model: "pawnContract" | "unsecuredContract" | "installmentContract";
