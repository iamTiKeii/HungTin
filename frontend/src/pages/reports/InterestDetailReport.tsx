    ...
    <div className="space-y-6 p-2 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 bg-slate-900/60 border border-slate-800 rounded-2xl p-2 px-3 backdrop-blur-md">
          ...
          <span className="text-slate-400 text-xs font-semibold">Chọn Năm:</span>
          <select
            value={year}
            onChange={(e) => setYear(Number(e.target.value))}
            className="bg-transparent text-slate-200 text-xs font-bold focus:outline-none border-none cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 backdrop-blur-md relative overflow-hidden group col-span-1">
          ...
          <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Tổng Lãi Thu Năm {year}</p>
      ...
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 backdrop-blur-lg space-y-4">
        <h3 className="text-lg font-bold text-slate-100">Biểu Đồ Lãi Thu Nhập Hàng Tháng (Năm {year})</h3>
        ...
        {loading ? (
          ...
          <div className="h-64 flex items-end justify-between gap-2 pt-8 pb-4 border-b border-slate-800/60 overflow-x-auto min-w-[600px] px-4">
            {chartData.map((item, idx) => {
              const heightPct = (Number(item.amount) / maxMonthValue) * 100;
              return (
                <div key={idx} className="flex flex-col items-center flex-1 group">
                  <span className="text-[10px] text-amber-400 font-bold mb-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
      ...
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 backdrop-blur-lg space-y-4">
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-bold text-slate-100">Chi Tiết Nhật Ký Đóng Tiền Lãi</h3>
          <button
            onClick={fetchData}
            ...
            <table className="table w-full text-slate-300">
              ...
              <tbody>
                {details.length === 0 ? (
                  ...
                  details.map((d) => (
                    <tr key={d.id} className="border-b border-slate-800/40 hover:bg-slate-900/20 text-xs">
                      <td>{new Date(d.transaction_date).toLocaleString("vi-VN")}</td>
                      <td className="font-semibold text-slate-200">{d.contract_code}</td>
                      <td>
                        <span className="badge badge-sm badge-warning badge-outline text-[10px] font-bold">
                          {d.type.includes("Cầm đồ") ? "Cầm đồ" : "Tín chấp"}
                        </span>
                      </td>
                      <td className="font-bold text-slate-100">{d.customer_name}</td>
                      ...
                      <td className="text-slate-400">{formatCurrency(d.other_amount)}</td>
                      <td className="text-amber-400 font-black">{formatCurrency(d.total_interest)}</td>
                    </tr>
                  ))
                )}
              </tbody>
