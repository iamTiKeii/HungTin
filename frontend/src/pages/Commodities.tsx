    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-6 rounded-2xl">
        ...
        <div className="flex gap-2">
          <button onClick={fetchCommodities} className="btn btn-outline border-slate-700 text-slate-300 btn-sm">
          ...
          {commodities.map((item) => (
            ...
            >
              <div>
                ...
                {item.description && (
                  <p className="text-sm text-slate-400 mt-4 leading-relaxed bg-slate-950/40 p-3 border border-slate-800/80 rounded-xl">
                    {item.description}
                  </p>
                )}
              </div>
              <div className="mt-6 pt-4 border-t border-slate-800/60 text-xs text-slate-500 font-semibold flex items-center gap-2">
        ...
        <div className="modal modal-open">
          <div className="modal-box bg-slate-900 border border-slate-800 text-slate-100 rounded-2xl">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                <label className="label text-slate-300 font-semibold text-sm">Tên loại tài sản *</label>
                <input
                  type="text"
              ...
              <div>
                <label className="label text-slate-300 font-semibold text-sm">Mã hệ thống (Ký tự viết liền không dấu) *</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="input input-bordered w-full pl-10 bg-slate-955 border-slate-800 text-slate-100 rounded-xl focus:border-amber-500"
                    required
                  />
                </div>
              </div>
              <div>
                <label className="label text-slate-300 font-semibold text-sm">Mô tả quy cách kiểm định thế chấp</label>
                <textarea
                  placeholder="Cần lưu trữ biển số xe, số khung, số máy khi tiếp nhận..."
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  className="textarea textarea-bordered w-full bg-slate-955 border-slate-800 text-slate-100 rounded-xl h-24"
                />
              </div>
